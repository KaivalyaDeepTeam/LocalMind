...
class ScoreChartGenerator:
    ...
    def create_score_gauge(score: float, max_score: float = 100, size: float = 120) -> Any:
        ...
        drawing.add(
            Circle(center_x, center_y, inner_radius, fillColor=COLORS["white"], strokeColor=None)
        )
            ...
            if score_width > 0:
                drawing.add(
                    Rect(
                        ...
                        y_pos,
                        max(score_width, 8),
                        bar_height,  # Minimum width for visibility
                        fillColor=color,
                        strokeColor=None,
                        rx=4,
                        ry=4,
    ...
    def create_mini_bar(percentage: float, width: float = 60, height: float = 8) -> Any:
        ...
        if fill_width > 0:
            drawing.add(
                Rect(
                    0, 0, max(fill_width, 4), height, fillColor=color, strokeColor=None, rx=2, ry=2
                )
            )
...
class PDFReportGenerator:
    ...
    def _build_header(self, file_name: str) -> list:
        ...
        # Main title
        elements.append(Paragraph("Audio Quality Audit Report", self._styles["Title"]))
        ...
        # Divider line
        elements.append(
            HRFlowable(width="100%", thickness=2, color=COLORS["primary"], spaceAfter=20)
        )
    ...
    def _build_detailed_scores_table(self, audit_result: dict[str, Any]) -> list:
        ...
        for param in scoring_details:
            ...
            table_data.append(
                [name, f"{score:.1f}", f"{max_score:.0f}", f"{percentage:.0f}%", rating]
            )
        ...
        if strengths or improvements:
            ...
            if strengths:
                strengths_content.append(
                    Paragraph(
                        "<font color='#10B981'><b>Strengths</b></font>", self._styles["SubHeading"]
                    )
                )
                for s in strengths[:5]:  # Limit to 5 items
                    strengths_content.append(
                        Paragraph(
                            f"<font color='#10B981'>&#10003;</font> {s}", self._styles["Strength"]
                        )
                    )
    ...
    def _build_transcript(self, audit_result: dict[str, Any]) -> list:
        ...
        for line in lines:
            ...
            if speaker:
                elements.append(Paragraph(f"<b>{speaker}:</b> {text}", self._styles["Transcript"]))
            else:
    ...
    def _build_footer(self) -> list:
        ...
        elements.append(
            HRFlowable(width="100%", thickness=1, color=COLORS["border"], spaceBefore=10)
        )
